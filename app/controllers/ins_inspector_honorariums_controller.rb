class InsInspectorHonorariumsController < ApplicationController
  before_action :set_model_inspector,  except: [:index, :new, :update, :create, :destroy] #Call the method set_model_instance before every action, but the index method. The index method needs a model array
  before_action :set_employee, only: [:index, :new, :show, :edit]
  
  
  MODEL_NAME = InsInspectorHonorarium #First letter uppercase
  SYMBOL_NAME = :ins_inspector_honorarium #lowercase and separated by underscore
  PARAMETERS = :ins_inspector_product_id #Used by the getter, getting the params from specific models
  ...
  def create
    ...
    @model_instance.honorarium_value = UnmaskMoney.new(params[:ins_inspector_honorarium]["honorarium_value"]).format
  ...
  def update
      ...
      if @model_instance.update_attributes(valid_params)  
         @model_instance.update(honorarium_value: UnmaskMoney.new(params[:ins_inspector_honorarium]["honorarium_value"]).format)
