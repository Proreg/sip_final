class SysStatesController < ApplicationController
  ...
  MODEL_NAME = SysState #First letter uppercase
  SYMBOL_NAME = :sys_state #lowercase and separated by underscore
  
  def index
    authorize! :show, params[:controller]
    @grid = initialize_grid(SysState, per_page: 4) #wice grid
  end
  ...
  def valid_params
    params.require(SYMBOL_NAME).permit(:description, :sys_country_id)#params require é usado para updates de varios valores. Nesse exemplo apenas um foi alterado
  end
