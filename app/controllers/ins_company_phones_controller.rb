class InsCompanyPhonesController < ApplicationController
  ...
  MODEL_NAME = InsCompanyPhone #First letter uppercase
  SYMBOL_NAME = :ins_company_phone #lowercase and separated by underscore
  PARAMETERS = :area_code, :phone_number, :sys_telephone_type_id, :ins_insurance_company_id  #Used by the getter, getting the params from specific models
 
  def index
    authorize! :show, params[:controller]     
    @grid = initialize_grid(MODEL_NAME.where(ins_insurance_company_id: params[:ins_insurance_company_id]), per_page: 4) #wice grid, showing only the relatives of a specific employee
  end
  ...
  def create
    ...
    @model_instance.ins_insurance_company_id = params[:ins_insurance_company_id] #SAve relative record with its employee
    a = @model_instance
