class InsSubInspectorPhonesController < ApplicationController
  ...
  before_action :set_sub_inspector, only: [:index, :new, :show, :edit]
  
  
  MODEL_NAME = InsSubInspectorPhone #First letter uppercase
  SYMBOL_NAME = :ins_sub_inspector_phone #lowercase and separated by underscore
  PARAMETERS = :area_code, :phone_number, :sys_telephone_type_id, :ins_sub_inspector_id  #Used by the getter, getting the params from specific models
  ...
  def create
    ...
    @model_instance.ins_sub_inspector_id = params[:inspector_id] #SAve relative record with its employee
    a = @model_instance
    if @model_instance.save
      flash[:msg_success] = 'Telefone do sub-inspetor cadastrado com sucesso.'
  ...
  def set_sub_inspector #set foreign keys variable descriptions
    @sub_inspector = InsSubInspector.find(params[:sub_inspector_id]) 
  end
end
