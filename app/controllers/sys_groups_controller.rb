class SysGroupsController < ApplicationController
before_action :set_model_instance, except: [:index, :new, :update, :create, :destroy] #Call the method set_model_instance before every action, but the index method. The index method needs a model array
  ...
  MODEL_NAME = SysGroup #First letter uppercase
  SYMBOL_NAME = :sys_group #lowercase and separated by underscore
  ...
  def create
    ...
    if @model_instance.save
      SetPermissions.new.perform(@model_instance.id)
      redirect_to :controller=> 'sys_permissions', :action=>'edit', :group_id=>@model_instance.id
    else
      redirect_to :action => 'index'
    end
