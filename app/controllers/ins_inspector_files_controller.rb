class InsInspectorFilesController < ApplicationController
  ...
  MODEL_NAME = InsInspectorFile #First letter uppercase
  SYMBOL_NAME = :ins_inspector_file #lowercase and separated by underscore
  PARAMETERS = :inspector_file, :description, :sys_document_type_id #Used by the getter, getting the params from specific models
  ...
  def download
    send_file InsInspectorFile.find(params[:id]).inspector_file.to_s
  end
