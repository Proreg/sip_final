class HrSchoolingLevelsController < ApplicationController
 before_action :set_model_instance, except: [:index, :new, :update, :create, :destroy] #Call the method set_model_instance before every action, but the index method. The index method needs a model array
  ...
  MODEL_NAME = HrSchoolingLevel #First letter uppercase
  SYMBOL_NAME = :hr_schooling_level #lowercase and separated by underscore
  ...
  def create
    ...
    if @model_instance.save
      flash[:msg_success] = 'Grau de escolaridade cadastrado com sucesso.'
