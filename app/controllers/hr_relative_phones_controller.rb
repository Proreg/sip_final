class HrRelativePhonesController < ApplicationController
  ...
  MODEL_NAME = HrRelativePhone #First letter uppercase
  SYMBOL_NAME = :hr_relative_phone #lowercase and separated by underscore
  PARAMETERS = :area_code, :phone_number, :sys_telephone_type_id, :hr_relative_id  #Used by the getter, getting the params from specific models
 
  def index
     authorize! :show, params[:controller]
     @grid = initialize_grid(MODEL_NAME.where(hr_relative_id: params[:relative_id]), per_page: 4) #wice grid, showing only the relatives of a specific employee
  end
  ...
  def create
    ...
    @model_instance.hr_relative_id = params[:relative_id] #SAve relative record with its relative
  a = @model_instance
    if @model_instance.save
      flash[:msg_success] = 'Telefone do parente cadastrado com sucesso.'
  ...
  def set_employee #set foreign keys variable descriptions
    ...
    @relative = HrRelative.find(params[:relative_id]) 
  end
end
